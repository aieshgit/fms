...
app.get("/vehicles", async (req, res) => {
  try {
    const allVehicles = await pool.query(
      `SELECT id, concat('V-',id) as "vehicleNum", reg_num as "regNum", vin, make, model, to_char(build_date, 'DD/MM/YYYY') as "buildDate" from vehicles`
    );
...
app.post("/drivers", async (req, res) => {
  try {
    ...
    const newDriver = await pool.query(
      "INSERT INTO drivers (name, username, email, phone, website) VALUES($1, $2, $3, $4, $5) RETURNING *",
...
app.get("/drivers/:id", async (req, res) => {
  try {
    //console.log(req.params);
    const { id } = req.params;
    const driver = await pool.query("SELECT * FROM drivers WHERE id = $1", [
      id,
    ]);
...
app.delete("/drivers/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const deleteDriver = await pool.query("DELETE FROM drivers WHERE id = $1", [
      id,
    ]);
